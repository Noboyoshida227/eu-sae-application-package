...
pbmcpeMFH3_with_existing <- function(formula, vardir, domain_var,
  ...
  varu2_vec <- sae_mfh_refvar_vector(result$fit$refvar, nT, "MFH3")
  ...
  while (b <= nB) {
    ...
    # Heteroskedastic AR(1) from the fitted msae/Molina-Romero model:
    # u_0 ~ N(0,1), u_t = rho*u_{t-1} + a_t, Var(a_t)=refvar[t].
    # The prior implementation initialized u_1=a_1 and omitted rho*u_0,
    # which understated the first-period variance and all cross-time
    # covariances whenever rho was non-zero.
    udt_mat_draw <- sae_simulate_mfh_random_effects(
      "MFH3", nD = nD, nT = nT, refvar = varu2_vec, rho = rho
    )
